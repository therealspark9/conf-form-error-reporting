    ...
    while (retries <= this.config.retryLimit) {
      try {
        ...
        page.on('request', (req) => {
          ...
          // Explicitly block PDF and Font extensions
        ...
        page.on('console', msg => {
          try {
            ...
            // Filter False Positives from aborted requests
        ...
        // Relaxed Timeout Strategy
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: this.config.timeout });
        
        // Wait buffer for JS execution
        ...
        // Handle intentional blocks (PDFs) as success
...
<body>
  ...
  <script>
    function switchTab(index) {
      ...
      // 4. Show target tab content by explicit ID
    ...
    function exportToExcel(tabIndex) {
      ...
      const safeName = tabNames[tabIndex] || 'report';
      
      link.href = URL.createObjectURL(blob);
      link.download = 'console-errors-' + safeName + '-' + timestamp + '.csv';
    ...
    try {
      const report = {
        ...
        summary: {
          ...
          urlsWithErrors: Array.from(this.pageErrors.values()).filter(p => p.errorCount > 0).length,
          totalErrors: Array.from(this.pageErrors.values()).reduce((sum, p) => sum + p.errorCount, 0),
          uniqueErrors: this.errorGroups.size
        },
        pages: Array.from(this.pageErrors.values()).map(page => ({
          url: page.url,
          status: !page.success ? 'failed' : page.errorCount > 0 ? 'error' : 'success',
          errorCount: page.errorCount,
          errors: page.errors
        })),
        uniqueErrorGroups: Array.from(this.errorGroups.values()).map(group => ({
          signature: group.signature,
          text: group.errorText,
          location: group.errorLocation,
          count: group.pages.size,
      ...
      return JSON.stringify({ error: 'Failed to generate JSON report' });
    }
  }
